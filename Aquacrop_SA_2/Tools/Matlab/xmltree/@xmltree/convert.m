...
switch type
    ...
    case 'chardata'
        ...
        %
        % Taking into account data types specified through a
        % type attribute
        att=attributes(tree,'get',parent(tree,uid));
        if isempty(att)
            return
        end
        % find if type attribute exists
        if isstruct(att)
            att={att};
        end
        
        idx=cellfun(@(x) strcmp(x.key,'type'),att);
        if isempty(idx)
            return
        else
            type=att{idx}.val;
